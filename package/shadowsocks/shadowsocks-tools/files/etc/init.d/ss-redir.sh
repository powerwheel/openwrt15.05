...
SS_TUNNEL_PIDFILE=/var/run/ss-tunnel-go.pid
...
start()
{
	...
	if [ -n "$vt_safe_dns" ]; then
		if [ "$vt_safe_dns_tcp" = 1 ]; then
            ...
            #start pdnsd
            start_pdnsd $vt_safe_dns || return 1
		else
			awk -vs="127.0.0.1#$SS_TUNNEL_PORT" '!/^$/&&!/^#/{printf("server=/%s/%s\n",$0,s)}' \
				/etc/gfwlist/$vt_gfwlist > /var/etc/dnsmasq-go.d/01-pollution.conf
            #start ss-tunnel
            /usr/bin/ss-tunnel -s$vt_server_addr -p$vt_server_port \
                -k"$vt_password" -m$vt_method -l$SS_TUNNEL_PORT -t$vt_timeout -f $SS_TUNNEL_PIDFILE -L$vt_safe_dns:$vt_safe_dns_port -u || return 1
		fi
	else
